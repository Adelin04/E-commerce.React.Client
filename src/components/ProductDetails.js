...
const ProductDetails = () => {
  ...
  useEffect(() => {
    if (products) {
      let res = products.filter((product) => product.id === parseInt(id));
      ...
      (await fetch(`${URI}basket/v1/add/newBasket`, {
        ...
        .then((data) => {
          ...
          if (success)
            newBasketCreated &&
              JSON.stringify(
                newBasketCreated.items.map((product) => {
                  TMP_BASKET.push({
  ...
  const AddNewItemToCart = () => {
    ...
    if (Object.keys(filterLocalStorage).length > 0) {
      ...
      localStorage_BASKET.push({
        productId: SerializeProduct(productById[0]).id,
        quantity: quantity,
        size: size,
      });
    }
    ...
    <div className="productDetails flex flex-col justify-between items-center w-full h-full">
      ...
      {productById && (
        <div className="container-product-details relative flex-col flex  justify-around items-center w-[90%] h-full my-5 mx-auto sm:flex-row">
          {msg && (
            <div className="msg absolute flex justify-center items-center top-0 left-0 right-0 w-max h-max">
              {msg}
            </div>
          )}
          ...
          <div className="wrapper-details-product flex flex-col justify-between items-center max-w-[80%] h-max p-1 m-1 sm:max-w-[40%] ">
            ...
            <div className="wrapper-title-price flex justify-around items-center w-full p-1 my-2 text-center bg-[var(--sliderColor)] rounded-md max-[350px]:m-7">
              <h3 className="product-title text-[25px] font-bold p-1 m-1 max-[300px]:text-[17px]">
                {productById[0].name}{" "}
              </h3>

              <div className="price-product-details flex justify-center items-center ">
                {<PriceFormated price={productById[0].price} />}
                <p className="currency flex justify-center items-center w-max font-bold text-lg">
                  {productById[0].currency}
                </p>
              </div>
            </div>
            ...
            <div className="wrapper-product-description text-lg flex flex-col justify-center items-center w-auto h-max p-1 m-1 sm:text-[15px]">
              <h4 className="description flex justify-center items-center">{`${productById[0].description
                .split()[0]
                .slice(0, 300)}...`}</h4>

    ...
    return (
        ...
        <div className=" flex justify-center items-center h-[300px] " >
            ...
            {photos && photos.map((photo: any, index: number) => {
                return (
                    <div className={`relative flex justify-center items-center h-full mx-2 rounded-2xl cursor-pointer overflow: "hidden"`} key={index} onClick={() => setIndexActive(index)}>

                        {photo ? <Image priority={true} objectFit="contain" className={`justify-center items-center relative h-full rounded-2xl  transition-all `} width={`${index === indexActive ? 300 : 50}`} height={300} src={photo.path.toString().trim()} alt="Image logo" /> : < FerrisWheelSpinner />}
